...
| 2026-09-16 | Instant App Launch & HTTP 400 Resolution: Removed artificial 2.2-second preloader delay in `AuthContext.jsx` for instant (<50ms) authentication resolution, reduced initial announcements background payload from 5000 to 300 in `useAnnouncements.js`, and fixed false HTTP 400 Bad Request returns in `ipoVerificationRoutes.js` for upstream registrar connection or timeout exceptions | `frontend/src/contexts/AuthContext.jsx`, `frontend/src/hooks/useAnnouncements.js`, `backend/routes/ipoVerificationRoutes.js` |
